...
class JobRegisterViewController: UIViewController {
    ...
    let hourPicker: UIButton = {
        ...
        button.addTarget(JobRegisterViewController.self, action: #selector(showHourPicker), for: .touchUpInside)
    ...
    lazy var descriptionTextField: UITextView = {
        ...
        textField.layer.cornerRadius = 12
        textField.text =
            "Buscamos alguém para integrar a equipe, com foco em bom atendimento, atenção aos detalhes e paixão pelo que faz.."
    ...
    lazy var bigStack: UIStackView = {
        var stack = UIStackView(arrangedSubviews: [
            stepStack, inputStack, descriptionStack, responsabilitiesStack, dutiesStack
        ])
        stack.translatesAutoresizingMaskIntoConstraints = false
        stack.axis = .vertical
        stack.spacing = 20
    ...
    @objc func validateForm() {
        ...
        let isFormValid =
            isTextFilled && isFunctionSelected && isDescriptionFilled && isResponsabilitiesFilled && isDutiesFilled
